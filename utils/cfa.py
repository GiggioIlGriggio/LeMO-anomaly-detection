...
class DSVDD(nn.Module):
    ...
    def forward(self, p):
        ...
        if self.training:
            ...
            elif self.loss == "tripletcentral":
                loss = self._triplet_central_loss(phi_p)
            elif self.loss == "online_nologexp":
                loss = self._online_loss_nologexp(phi_p)
            elif self.loss == "NCE":
                loss = self._NCE_loss(phi_p)
            elif self.loss == "email":
                loss = self._email_loss(phi_p)
            elif self.loss == "NCENEW":
                loss = self._NCE_loss_NEW(phi_p)
            else:
                print("ATTENZIONE LOSS NON TROVATA")
    ...
    def _NCE_loss(self, features):
        ...
        d_clone[:, self.positives:] = 1 - d_clone[:, self.positives:] #REMOVE THIS LINE
    ...
    def _NCE_loss_NEW(self, features):
        #1 positive sample and 3 negative
        memory_bank = self.C.transpose(0, 1)
        num_prototypes = memory_bank.shape[0]
        ...
        positives = 2
        negatives = 4
        d[:, positives : positives+negatives] = -1 * d[:, positives : positives+negatives]

        d = torch.sigmoid(d)#torch.softmax(d, dim=1)
        ...
        log  = torch.log(d) #torch.log(d_clone)
    ...
    def _NCE_softmax_loss(self, features):
        ...
        # Perform the operation on the copy
        d_clone[:, self.positives:] = 1 - d_clone[:, self.positives:] #try to change remove this line
    ...
    def _triplet_central_loss_OLD(self, phi_p):
        ...
        loss = triplet + closest_distance #TODO Implementare un fattore alpha per bilanciare le loss
    ...
    def _triplet_central_loss(self, phi_p):
        ...
        L_att = torch.sum(dist[:, : , :self.positives])

        L_rep  = torch.sum(dist[:, : , self.positives:])
        
        closest_distance = torch.mean(dist[:, : , 0])
        
        alpha = 1
        triplet = (L_att - L_rep)/phi_p.shape[1] + alpha

        triplet = torch.max(triplet,torch.zeros_like(triplet))
        
        loss = triplet + closest_distance * 0.1
        return loss
    
    def _email_loss(self, features): #ANGULAR LOSS
